...
model = torch.hub.load("ultralytics/yolov5", "custom", path="path/to/visdrone.pt")  # local model
model = torch.hub.load("path/to/yolov5", "custom", path="path/to/visdrone.pt", source="local")  # local repo
```
