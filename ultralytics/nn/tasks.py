...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11s.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
...
def torch_safe_load(weight, safe_only=False):
    ...
    ckpt, file = torch_safe_load("path/to/visdrone.pt", safe_only=True)
    ```
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in frozenset({Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, AFPNHead}):
