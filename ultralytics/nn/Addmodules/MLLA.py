...
def test_C2PSAMLLA():
    ...
    num_layers = 3  # PSA模块堆叠次数
    expansion = 0.5  # 扩展因子


    c1 = c2 = base_channels
    model = C2PSAMLLA(c1, c2, n=num_layers, e=expansion)

    # ---------------------------
    # 测试案例1：标准输入
    # ---------------------------
    # 模拟YOLO特征图输入 (batch, channels, H, W)
    x1 = torch.randn(batch_size, c1, *img_size)
    # 前向推理
    out1 = model(x1)
    # 验证输出形状与输入一致
    assert out1.shape == x1.shape, f"Shape mismatch: input {x1.shape}, output {out1.shape}"
    print(f"Test Case 1 Passed! Input/Output shape: {x1.shape} -> {out1.shape}")
