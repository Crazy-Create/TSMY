...
from ultralytics.nn.Addmodules.pscan import pscan   #注意替换成你的路径
...
class C2VMamba(nn.Module):
    def __init__(self, c1, c2, n=1, d_state=16, expand_factor=2, d_conv=4, bidirectional=True):
        """
        YOLO 适配器：将 VMambaBlock 封装为类似 C2 模块的形式

        参数:
            c1: 输入通道数
            c2: 输出通道数 (VMambaBlock 要求 c1 == c2)
            n: VMambaBlock 堆叠次数
            d_state: 状态维度
            expand_factor: 内部扩展因子
            d_conv: 卷积核大小
            bidirectional: 是否双向扫描
        """
        super().__init__()

        # 确保输入输出通道数相同
        if c1 != c2:
            raise ValueError(f"VMambaBlock requires c1 == c2, got c1={c1}, c2={c2}")
        ...
        for _ in range(n):
            ...
            self.layers.append(VMambaBlock(config))

    def forward(self, x):
        """
        前向传播，支持 4D 图像输入 (B, C, H, W)
        """
        # 保存原始尺寸
        B, C, H, W = x.shape

        # 转换为序列形式 (B, C, H, W) -> (B, H*W, C)
        ...
        # 转换回图像形式 (B, H*W, C) -> (B, C, H, W)
        output = x_seq.reshape(B, H, W, C).permute(0, 3, 1, 2)
        #return output
...
def test_C2VMamba():
    # 定义测试参数
    batch_size = 2  # 测试批次大小
    base_channels = 256  # 基础通道数(YOLO常用256/512)
    img_size = (64, 64)  # 特征图尺寸(H,W)
    num_layers = 3  # VMambaBlock堆叠次数

    # 创建模型 (c1 和 c2 必须相同)
    model = C2VMamba(c1=base_channels, c2=base_channels, n=num_layers)

    # 模拟YOLO特征图输入 (batch, channels, H, W)
    x1 = torch.randn(batch_size, base_channels, *img_size)

    # 前向推理
    out1 = model(x1)

    # 验证输出形状与输入一致
    assert out1.shape == x1.shape, f"Shape mismatch: input {x1.shape}, output {out1.shape}"
    print(f"✅ 测试通过: 输入 {x1.shape} -> 输出 {out1.shape}")
